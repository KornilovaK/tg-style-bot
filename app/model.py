...
class Normalization(nn.Module):
    def __init__(self, mean, std):
        ...
        self.mean = torch.tensor(mean).view(-1, 1, 1) # self.mean = mean.clone().detach().view(-1, 1, 1)
        self.std = torch.tensor(std).view(-1, 1, 1) # self.std = mean.clone().detach().view(-1, 1, 1)
